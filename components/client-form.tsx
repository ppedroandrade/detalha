...
export function ClientForm({
  ...
  open: boolean;
  onClose: () => void;
  onCreate: (values: { name: string; email: string; password: string; projectName: string }) => void | Promise<void>;
  ...
  const submit = async (event: FormEvent) => {
    ...
    try { await onCreate({ name, email, password, projectName }); } catch (error) { setError((error as Error).message); return; }
    ...
    <Dialog open={open} onClose={onClose} title="Novo cliente" description="O cliente receberá acesso a um projeto próprio com os ambientes iniciais.">
      <form onSubmit={submit}>
        <div className="space-y-5 px-5 py-6 sm:px-7">
          ...
          <div><Label>Senha inicial *</Label><Input type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={12} placeholder="Mínimo de 12 caracteres" required /></div>
